...
describe('case_tag_test', () => {
  ...
  it('test_case_on_size_with_else', () => {
  ...
  it('test_case_on_length_with_else', () => {
  ...
  it('test_assign_from_case', () => {
    ...
    assert.equal('menswear', template.render_strict({ collection: { handle: 'menswear-jackets' } }));
    assert.equal('menswear', template.render_strict({ collection: { handle: 'menswear-t-shirts' } }));
    assert.equal('womenswear', template.render_strict({ collection: { handle: 'x' } }));
    assert.equal('womenswear', template.render_strict({ collection: { handle: 'y' } }));
    assert.equal('womenswear', template.render_strict({ collection: { handle: 'z' } }));
  });

  it('test_case_when_or', () => {
    ...
    assert_template_result(' its 1 or 2 or 3 ', code, { condition: null });
    assert_template_result('', code, { condition: 'something else' });
  });

  it('test_case_when_comma', () => {
  ...
  it('test_case_when_comma_with_assign', () => {
    const code = handle => `
    ...
    assert.equal(render_strict(code('cake')).trim(), 'This is a cake');
    assert.equal(render_strict(code('cookie')).trim(), 'This is a cookie');
    assert.equal(render_strict(code('biscuit')).trim(), 'This is a cookie');
    assert.equal(render_strict(code('foo')).trim(), 'This is not a cake nor a cookie');
  });

  it('test_case_when_comma_and_blank_body', () => {
    const code = '{% case condition %}{% when 1, 2 %} {% assign r = "result" %} {% endcase %}{{ r }}';
    assert_template_result('  result', code, { condition: 2 });
  });

  it('test_case_detects_bad_syntax', () => {
    assert.throws(() => {
