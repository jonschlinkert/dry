...
describe('output_test', () => {
  ...
  it('test_variable', () => {
    ...
    const expected = ' bmw ';
    assert.equal(expected, render(text, assigns));
  });

  it('test_variable_traversing_with_two_brackets', () => {
    const text = '{{ site.data.menu[include.menu][include.locale] }}';
    assert.equal('it works!', render(text, {
      site: { data: { menu: { foo: { bar: 'it works!' } } } },
      include: { menu: 'foo', locale: 'bar' }
    }));
  });

  it('test_variable_traversing', () => {
  ...
  it('test_variable_piping', () => {
    const text = ' {{ car.gm | make_funny }} ';
    const expected = ' LOL ';

    assert.equal(expected, render(text, assigns, { filters: [FunnyFilter] }));
  });

  it('test_variable_piping_with_input', () => {
  ...
  it('test_variable_piping_with_args', () => {
    const text = ' {{ car.gm | add_smiley : ":-(" }} ';
    const expected = ' bad :-( ';
    assert.equal(expected, render(text, assigns, { filters: [FunnyFilter] }));
  });

  it('test_variable_piping_with_no_args', () => {
    const text = ' {{ car.gm | add_smiley }} ';
    const expected = ' bad :-) ';
    assert.equal(expected, render(text, assigns, { filters: [FunnyFilter] }));
  });

  it('test_multiple_variable_piping_with_args', () => {
    const text = ' {{ car.gm | add_smiley : ":-(" | add_smiley : ":-(" }} ';
    const expected = ' bad :-( :-( ';
    assert.equal(expected, render(text, assigns, { filters: [FunnyFilter] }));
  });

  it('test_variable_piping_with_multiple_args', () => {
  ...
  it('test_variable_piping_with_variable_args', () => {
