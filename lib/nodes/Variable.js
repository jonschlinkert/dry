...
const { strict_parse_with_error_mode_fallback } = require('../select-parser');
...
class Variable extends Node {
  ...
  render(context) {
    const value = this.render_variable(context);

    if (Array.isArray(value)) {
      // output += value.join('');
    } else if (value != null && value.to_liquid) {
      // output += String(value);
    }
