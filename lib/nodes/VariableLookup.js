...
const { isInteger, isObject, scan, to_liquid, to_liquid_value } = require('../utils');
...
class VariableLookup {
  ...
  resolve(context, prop) {
    for (const scope of context.environments) {
      const value = get(scope, prop);
      if (value !== undefined) {
        return to_liquid(value);
      }
    }
  }
  ...
  evaluate(context) {
    ...
    for (let i = 0; i < this.lookups.length && object != null; i++) {
      const lookup = this.lookups[i];
      const key = to_liquid_value(context.evaluate(lookup));
    ...
    if (object && object.to_liquid_value) {
