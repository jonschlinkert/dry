...
class Conditional {
  ...
  interpret_condition(left, right, operator, context) {
    ...
    if (!operator) {
      ...
      if (typeof output === 'string') output = Conditional.parse_expression(output);
      return output;
    }
    ...
    const op = operators[operator] || raise();

    if (typeof op === 'function') {
      return op(left, right, operator, this);
    }

    // console.log(this)
    // if (left[op] && right[op] && !isPlainObject(left) && !isPlainObject(right)) {
    if (left[op] && right[op]) {
      try {
...
class Main extends Conditional {
  name = 'main';
}
