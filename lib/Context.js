...
const { handlers, isNil, isPlainObject, isObject, isPrimitive, resolve, toString } = utils;
...
class Context {
  ...
  constructor({
    ...
    this.static_environments = [static_environments].flat().filter(Boolean);
    ...
    this.partial = false;
    this.profiler = null;
    this.strict_root_variables = false;
  ...
  push(new_scope = {}) {
    if (this.scopes.length >= Dry.MAX_DEPTH) {
  ...
  new_isolated_subcontext() {
    ...
    if (this.recursion_level > this.MAX_RECURSION) {
      throw new Dry.StackLevelError('Maximum recursion depth exceeded');
    }
    ...
    subcontext.recursion_level = this.recursion_level + 1;
  ...
  get(expression) {
    const env = this.environments.find(e => isObject(e) ? expression in e : !isNil(e[expression]));
  ...
  async try_variable_find_in_environments(key, raise_on_not_found) {
    ...
    if (this.strict_variables && raise_on_not_found) {
      throw new Dry.UndefinedVariable(`undefined variable ${key}`);
    }
  }
  ...
  async squash_instance_assigns_with_environments() {
    const scope = this.scope;
    const squashed = new Set();

    for (const k of Object.keys(scope)) {
      if (squashed.has(k)) continue;
      squashed.add(k);

      const env = this.environments.find(env => isObject(env) ? k in env : !isNil(env[k]));
      if (env) {
        try {
          scope[k] = await this.lookup_and_evaluate(env, k);
        } catch (err) {
  ...
  resolve_variable(variable, obj, key) {
    if (isObject(variable) && this.should_add_context(variable)) {
