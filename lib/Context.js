...
class Context {
  ...
  constructor({
    ...
    this.exception_renderer = Template.default_exception_renderer;
    if (rethrow_errors) {
      this.exception_renderer = Dry.RAISE_EXCEPTION_LAMBDA;
    }
  ...
  // Merge a hash of variables into the current local scope
  merge(new_scopes) {
  ...
  evaluate(object) {
    ...
    const resolve = v => {
      if (v instanceof Dry.Error) {
        const line_number = this.state?.line_numbers && object?.line_number;
        this.handle_error(v, line_number ?? this.state?.loc.line);
      }

      if (typeof v === 'symbol') return Dry.utils.toString(v);
