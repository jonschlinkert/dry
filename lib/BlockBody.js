...
class BlockBody {
  ...
  FullToken           = r('m')`^${TagStart}${WhitespaceControl}?(\\s*)(\\w+)(\\s*)(.*?)${WhitespaceControl}?${TagEnd}$`;
  ContentOfVariable   = r('m')`^${VariableStart}${WhitespaceControl}?(.*?)${WhitespaceControl}?${VariableEnd}$`;
  ...
  static raise_missing_tag_terminator(token, state) {
    throw new SyntaxError(state.locale.t('errors.syntax.tag_termination', { token, tag_end: TagEnd.source }));
  }

  // @api private
  static raise_missing_variable_terminator(token, state) {
    throw new SyntaxError(state.locale.t('errors.syntax.variable_termination', { token, tag_end: VariableEnd }));
  }
  ...
  parse_for_document(tokenizer, state, block) {
    ...
    while ((token = tokenizer.shift())) {
      ...
      if (token.startsWith(this.TAGSTART)) {
        this.whitespace_handler(token, state);

        if (!this.Syntax(token, this.FullToken)) {
        ...
        this.blank &&= new_tag.blank;
        this.nodelist.push(new_tag);
      } else if (token.startsWith(this.VARSTART)) {
        this.whitespace_handler(token, state);
        ...
        this.blank &&= this.WhitespaceOrNothing.test(token);
      }
  ...
  whitespace_handler(token, state) {
    ...
    state.trim_whitespace = (token[token.length - 3] === WhitespaceControl);
  }
  ...
  create_variable(token, state) {
    const match = this.ContentOfVariable.exec(token);
