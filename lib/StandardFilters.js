...
// const HTML_ESCAPE_ONCE_REGEXP = new RegExp(HTML_ESCAPE_SOURCE);
...
// Based on https://github.com/k0kubun/ruby/blob/ffd0820ab317542f8780aac475da590a4bdbc7a8/array.c
exports.escape = exports.h = input => {
...
exports.url_encode = input => input ? encodeURIComponent(toString(input)) : input;
exports.url_decode = input => {
  return input ? decodeURIComponent(toString(input).replace(/\+/g, ' ')) : input;
};
...
exports.atob = input => {
  try {
    ...
    throw new Dry.ArgumentError('invalid base64 provided to base64_decode');
  }
};

exports.base64_encode = input => {
  // const output = encodeURIComponent(toString(input))
  //   .replace(/%([0-9A-F]{2})/g, (_, $1) => {
  //     return String.fromCharCode('0x' + $1);
  //   });

  return exports.btoa(input);
};

exports.base64_decode = input => {
  try {
    // const output = [...exports.atob(toString(input))].reduce((acc, ch) => {
    //   return acc + (`%00${ch.charCodeAt(0).toString(16)}`).slice(-2);
    // }, '');

    // return decodeURIComponent();
    return exports.atob(input);
  } catch (err) {
    throw new Dry.ArgumentError('invalid base64 provided to base64_decode');
  }
};

exports.base64_url_safe_encode = input => {
  // return Base64.urlsafe_encode64(input.to_s);
  return exports.base64_encode(input);
};

exports.base64_url_safe_decode = input => {
  try {
    return exports.base64_decode(input);
    // return Base64.urlsafe_decode64(input.to_s);
  } catch (err) {
    throw new Dry.ArgumentError('invalid base64 provided to base64_url_safe_decode');
  }
};
...
exports.slice = (input, offset, length) => {
  ...
  if (offset !== undefined && !utils.isNumber(offset)) {
    throw new errors.ArgumentError('expected offset to be a number');
  }

  if (length !== undefined && !utils.isNumber(length)) {
    throw new errors.ArgumentError('expected length to be a number');
  }
...
exports.sort = function(input, prop = null) {
  ...
  if (prop && Array.isArray(input) && input.length > 0) {
    if (!utils.isNumber(prop) && !input.some(v => utils.isObject(v))) {
      throw new errors.ArgumentError('invalid property: ' + String(prop));
    }
  }
...
exports.sort_natural = function(input, property = null) {
  if (property && Array.isArray(input) && input.length > 0) {
    if (!utils.isNumber(property) && !input.some(v => utils.isObject(v))) {
      throw new errors.ArgumentError('invalid property: ' + String(property));
    }
  }
...
exports.where = function(input, property, target_value = null) {
  if (typeof input === 'number' || typeof input === 'symbol') {
    throw new errors.ArgumentError('input is not iterable: ' + String(input));
  }
...
exports.map = function(input, prop) {
  const value = null//this && this?.context?.lookup_variable(prop);

  const resolve = ele => {
    if (typeof ele === 'function') ele = ele();
    if (prop === 'to_liquid') return ele;
    if (typeof ele?.to_liquid === 'function') ele.to_liquid();
    if (Array.isArray(ele)) return ele.map(e => resolve(e));
    if (!utils.isNil(ele)) {
      const r = ele[prop];
      if (r !== undefined) {
        return typeof r === 'function' ? r() : r;
      }
      if (utils.isPrimitive(input)) return value;
      return value == null ? ele : value;
    }
    return ele;
  };

  if (input != null && typeof input.map === 'function' && !(input instanceof Dry.Drop)) {
    return input.map(ele => resolve(ele));
  }
...
//   const resolve = v => {
//     if (v?.to_liquid) v = v.to_liquid();
//     if (v?.to_s) v = v.to_s();
//     return v;
//   };

//   // if (input && input[prop] && typeof input[prop] === 'function') {
//   //   return resolve(input[prop]());
//   // }

//   // if (input?.each) {
//   //   const arr = [];
//   //   input.each(ele => ele != null && prop in ele && arr.push(ele[prop]));
//   //   return arr;
//   // }
...
//   // if (prop === 'to_liquid' && input?.to_liquid) {
...
//     //   if (utils.isObject(e) && prop && prop in e) {
//     //     const v = e[prop];
//     //     result.push(typeof v === 'function' ? v() : v);
//     //     return;
//     //   }

//     //   if (e?.constructor?.name.includes('Drop') && typeof e.each === 'function') {
//     //     result.push(e.each((ele, i) => ele[prop]).join(''));
//     //     return;
//     //   }

//     //   if (prop && (!e || e[prop] === undefined)) {
//     //     return e;
//     //   }

//     //   result.push(resolve(e));
//     // });

//     // const output = result;

//     // if (output.every(ele => typeof ele === 'string')) {
//     //   Reflect.defineProperty(output, 'toString', { value: () => output.join('') });
//     // }
...
exports.concat = function(input, array) {
  if (!Array.isArray(array)) {
    throw new errors.ArgumentError('concat filter requires an array argument');
  }
...
exports.date = (input, format) => {
  ...
  if (/^[0-9]+$/.test(input) && toString(input).length === 10) {
    input = toString(input) + '000';
  }

  if (toString(input).toLowerCase() === 'today' || toString(input).toLowerCase() === 'now') {
    input = new Date();
  }

  const date = typeof input === 'string' ? utils.to_date(input) : input;
...
const assert_non_zero = value => {
  if (utils.to_number(value) === 0) {
    throw new errors.ZeroDivisionError('cannot divide by zero');
  }
};
...
const raise_property_error = (err, property) => {
  console.error(err);
  throw new errors.ArgumentError(`cannot select the property '${property}'`);
};

// const apply_operation = (input, operand, operation) => {
//   return utils.to_number(input).send(operation, utils.to_number(operand));
// };
...
class InputIterator {
  constructor(input, context) {
    ...
    // this.drop = input instanceof Dry.Drop;
    ...
    //   this.input = [input];
    // } else if (!utils.isIterable(input)) {
    //   this.input = utils.toArray(input);
    // } else {
    //   this.input = input;
    // }
