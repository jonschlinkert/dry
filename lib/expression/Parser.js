...
class Parser {
  ...
  eos() {
    return this.p >= this.tokens.length - 1;
  }
  ...
  consume(type = null) {
    ...
    if (type && token[0] !== type) {
      throw new SyntaxError(`Expected ${type} but found ${this.tokens[this.p][0]}`);
    }
  ...
  expression() {
    ...
    switch (token[0]) {
      ...
      default: {
        throw new SyntaxError(`"${token[1]}" is not a valid expression`);
      }
    }
  }
