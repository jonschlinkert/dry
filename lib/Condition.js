...
const operators = {
  ...
  '!==': (left, right, op, node) => !node.equal_variables(left, right, op, { strict: true }),
  '<>': (left, right, op, node) => !node.equal_variables(left, right, op),
  '<=>': (left, right, op, node) => { throw new SyntaxError('Not implemented yet: ends_with'); },
  contains: (left, right) => {
...
class Condition {
  ...
  equal_variables(left, right, operator, { strict = false } = {}) {
    if (left instanceof Expression.MethodLiteral) {
    ...
    if (right instanceof Expression.MethodLiteral) {
