...
class Condition {
  ...
  async interpret_condition(left, right, operator, context, previous_result) {
    ...
    const defined = right instanceof Dry.VariableLookup && right.markup === 'defined';
    left = await context.evaluate(left);
    right = await context.evaluate(right);

    if (defined === true && right === undefined && (operator === 'is' || operator === 'isnt')) {
      return operator === 'is' ? left !== undefined : left === undefined;
    }
