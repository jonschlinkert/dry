...
class Expression {
  ...
  static SINGLE_QUOTED_STRING = /^\s*'([\s\S]*?)'\s*$/m;
  static DOUBLE_QUOTED_STRING = /^\s*"([\s\S]*?)"\s*$/m;
  ...
  static parse(value) {
    ...
    if (exec(this.SINGLE_QUOTED_STRING)) return this.match[1];
    if (exec(this.DOUBLE_QUOTED_STRING)) return this.match[1];
