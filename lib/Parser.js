...
class Parser {
  ...
  push(node, append = true) {
    if (!node) return;
    const { block, prev, stack } = this;

    if (this.depth > Dry.MAX_DEPTH) {
      throw new Dry.StackLevelError('Maximum nesting depth exceeded');
    }

    // Track nodes being processed to prevent race conditions
    if (this.pendingNodes.has(node)) {
      throw new Dry.SyntaxError('Circular node reference detected');
    }
    this.pendingNodes.set(node, true);
