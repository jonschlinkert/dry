...
class Parser {
  ...
  capture_tag(token) {
    ...
    switch (token.name) {
      ...
      case `end${this.block.name}`: return this.pop(new Close(token, state, block));
      default: {
  ...
  capture_comment(token) {
    ...
    const CommentTag = this.tags.get('comment');
    this.push(new CommentTag(undefined, this.state, this.block));
    this.push(token);
  }
  ...
  push(node) {
    ...
    if (node.type === 'open') utils.trim_whitespace(node, this);
    if (node.type === 'close') utils.trim_whitespace(node, this);
