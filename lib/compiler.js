...
Compiler.prototype.init = function() {
  ...
  this.set('extends', function(node) {
    ...
    // var parent = this.getFile(this.file, node.type);
    // this.mergeBlocks(parent, this.file);
    return '';
  });
...
Compiler.prototype.mapVisit = function(nodes, delim) {
  ...
  while (++idx < len) {
    ...
    if (typeof fn === 'function') {
      ...
      ctx.fn = function() {
        // placeholder for compiled fn
        return str;
      };
      str = fn.apply(ctx, node.args);
    }
  }
  return str;
};
...
Compiler.prototype.mergeBlocks = function(parent, file, fn) {
  var blocks = parent.ast.blocks;
  var child = file.ast.blocks;

  for (var key in blocks) {
    if (blocks.hasOwnProperty(key) && child.hasOwnProperty(key)) {
      var block = blocks[key];

      for (var prop in block) {
        if (block.hasOwnProperty(prop) && child[key].hasOwnProperty(prop)) {
          // block[prop] = utils.clone(child[key][prop]);
          var tok = utils.clone(child[key][prop]);
          var node = utils.clone(block[prop]);
          block[prop] = this.updateNodes(tok, node);
        }
      }
    }
  }
...
Compiler.prototype.callHelper = function(node, helper) {
  ...
  return helper.apply(thisArg, utils.parseArgs(node.args));
};
