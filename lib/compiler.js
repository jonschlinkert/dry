...
function applyLayouts(file, options, cache) {
  var opts = utils.extend({}, options);
  ...
  if (layout.layout) {
    applyLayouts(layout, options, cache);
  }
...
function replaceNodes(parent, child) {
  var blocks = child.ast.blocks.block;
  var nodes = parent.ast.nodes;
  var len = nodes.length;
  var idx = -1;

  while (++idx < len) {
    var node = nodes[idx];
    if (node.type === 'block' && !child.ast.isLayout && blocks[node.name]) {
