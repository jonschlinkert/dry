...
const { deleteProperty, get, has, set } = Reflect;
...
exports.handlers = {
  set(target, key, value, receiver) {
    if (key !== 'undefined') {
      return has(target, key) ? set(target, key, value, receiver) : target.set(key, value);
    }
  },
  get(target, key, receiver) {
    return has(target, key) ? get(target, key) : typeof target.get === 'function' && target.get(key);
  },
  has(target, key) {
    return typeof target.has === 'function' ? target.has(key) : has(target, key);
  },
  ownKeys(target) {
    return typeof target.keys === 'function' ? target.keys() : Reflect.ownKeys(target);
  },
...
exports.toString = value => {
  if (typeof value === 'symbol') return value.toString().slice(7, -1);
...
exports.unindent = input => {
  const lines = input.replace(/^\s*\n|\n\s*$|^\s+$/g, '').replace(/\t/g, '  ').split(Dry.regex.Newline);
  ...
  if (exports.isPlainObject(collection)) {
    collection = Object.entries(collection).map(([key, value]) => {
      if (exports.isObject(value)) {
        ...
        // if a custom `toString` method was not provided, create one
        if (Object.prototype.toString === value.toString) {
          value.toString = () => exports.isPrimitive(value) ? value : JSON.stringify(value);
        }
      }
      return [key, value];
    });
