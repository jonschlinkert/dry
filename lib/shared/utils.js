...
exports.trim_whitespace_left = (node, prev = node.prev) => {
...
exports.trim_whitespace_right = (node, next = node.next) => {
  if (node && node.trim_right && next && next.value) {
    next.value = next.value.trimStart();
  }
};

exports.trim_whitespace = node => {
  exports.trim_whitespace_right(node);
