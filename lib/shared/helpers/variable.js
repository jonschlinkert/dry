...
exports.set_line_number = (node, state) => {
  if (Dry.utils.isObject(node) && node.line_number === undefined && state && state.loc) {
    if (state.loc.line) {
...
exports.parse_variable_strict = (markup, variable = {}) => {
  ...
  if (variable && variable.state.line_numbers === true) {
    exports.set_line_number(name, variable);
  }
...
exports.render_macro = async (variable_lookup, macro_args = [], context) => {
  await setImmediate();

  const { name, lookups } = variable_lookup;
  let macro = await context.evaluate(variable_lookup);

  if (!macro && name === 'macros' && lookups.length > 0) {
    variable_lookup.name = variable_lookup.lookups.shift();
    macro = await context.evaluate(variable_lookup);
  }

  if (!macro && name?.markup === 'block') return exports.render_block(macro_args, context);
  ...
  for (let index = 0; index < macro.params.length; index++) {
    ...
    const variable = await Dry.utils.resolve(args[index], context);
    const value = await context.evaluate(variable);
...
exports.render_variable = async (name, filters = [], context) => {
  let value = await context.evaluate(name);
