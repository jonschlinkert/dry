...
const REGEX_BOM         = /^\ufeff/;
const REGEX_COMMENT_TAG = /^({#)(-?)\s*([\s\S]*?)\s*(-?)#}/;
const REGEX_ESCAPED     = /^\\+/;
...
class Lexer {
  constructor(input, options) {
    ...
    this.tokens = [];
    this.queue = [];
    // Convert input to Buffer to properly handle UTF-8
    this.source = Buffer.from(input);
    this.remaining = this.source.toString('utf8');
    this.consumed = '';
    this.inside = 0;
  ...
  captureVariable() {
    return this.capture(REGEX_VARIABLE, 'variable');
  }
  ...
  advance() {
    const output = this.loc.index === 0 && this.captureBom()
      || this.captureEscaped()
