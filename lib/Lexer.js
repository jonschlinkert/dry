...
class Lexer {
  ...
  captureDotDot() {
    return this.inside > 0 && this.capture(REGEX_DOTDOT, 'dotdot');
  }
  ...
  //   const match = /^([\s\S]*?)({%\s*(endraw)\s*%})/.exec(this.lexer.remaining);
  //   if (!match) {
  //     throw new Dry.SyntaxError('{% raw %} tag is missing a closing {% rawend %} tag');
  //   }
  ...
  // captureCommentTag() {
  //   return this.capture(REGEX_COMMENT_TAG, 'comment_tag');
  // }

  // captureRawTag() {
  //   return this.capture(REGEX_RAW_TAG, 'raw_tag');
  // }
  ...
  captureTag() {
    ...
    if (token) {
      ...
      this.skipWhile(tok => {
        ...
        if (!token.name) {
          if (tok.type === 'text') {
            token.name = token.tag_name = tok.value;
          }
        } else if (tok.type !== 'tag_close' && (tok.type !== 'space' || token.name === 'liquid')) {
          args.push(tok.value);
        }
      ...
      if (last && last.loc && token.loc) {
        token.loc.end = last.loc.end;
      }
  ...
  //   const token = this.capture(REGEX_TEXT, 'text') || this.captureChar();
  ...
  captureTagOpen() {
    const token = this.capture(REGEX_TAG_OPEN, 'tag_open');
  ...
  advance() {
    return this.captureBom && this.captureBom()
      || this.captureEscaped()
      || this.capture(REGEX_COMMENT_TAG, 'comment_tag')
      || this.capture(REGEX_RAW_TAG, 'raw_tag')
      || this.captureTag()
      // || this.captureTagOpen()
  ...
  skipWhile(fn = () => !this.eos()) {
    const skipped = [];
    let n = 0;
    while (fn(this.peek(), n++)) skipped.push(this.dequeue() || this.advance());
    return skipped;
  }
