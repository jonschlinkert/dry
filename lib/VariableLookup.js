...
class VariableLookup {
  ...
  constructor(input) {
    ...
    Dry.utils.scan(input, Dry.regex.VariableParser, (match, index) => {
      if (!this.name) {
  ...
  evaluate(context) {
    ...
    const invalid = (k, i) => {
      ...
      if (Dry.helpers.methods[k] && (!bracketed || bracketed.index !== i)) {
        return Dry.helpers.methods[k](object);
      }
    };
    ...
    for (let i = 0; i < this.lookups.length && object != null; i++) {
      ...
      if (object instanceof Dry.Drop && !('index' in object)) {
        if (Dry.constants.PROTECTED_KEYS.has(k)) return null;
        if (Dry.constants.DROP_KEYS.has(k)) return k;
