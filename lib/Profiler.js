...
class Profiler {
  ...
  profile(template_name, options, block) {
    // nested renders are done from a tag that already has a timing node
    if (this.current_children) return block();
    ...
    try {
      ...
      this.profile_node(template_name, options, block);
    } catch (err) {
  ...
  profile_node(template_name, { code = null, line_number = null } = {}, block) {
    const timing = new Timing({ code, template_name, line_number });
    const parent_children = this.current_children;
