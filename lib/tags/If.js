...
const {
  constants,
  regex: { QuotedFragment: q },
  shared,
  utils
} = Dry;

class If extends Dry.BlockTag {
  static BOOLEAN_OPERATORS = new Set(['and', 'or']);
  ...
  lax_parse(markup = this.markup) {
    ...
    while (expressions.length > 0) {
      ...
      if (!If.BOOLEAN_OPERATORS.has(operator)) return this.raise_syntax_error('if');
  ...
  parse_expression(markup = this.markup) {
    const expr = Condition.parse_expression(this.state, markup);
  ...
  async render_to_output_buffer(context) {
    try {
      for (const branch of this.branches) {
        ...
        const expression = await this.parse_args(branch, context);
        branch.condition = await this.parse(expression);
        // const result = await expressions.evaluate(expression, context);
