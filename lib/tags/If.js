...
const { BlockTag, Condition } = require('../nodes');
...
class If extends BlockTag {
  ...
  lax_parse(markup = this.markup) {
    ...
    let match = last && IF_SYNTAX.exec(last.input.trim());

    if (!match) throw new SyntaxError(this.state.locale.t('errors.syntax.if'));
    ...
    while (expressions.length) {
      ...
      if (!BOOLEAN_OPERATORS.has(operator)) {
        throw new SyntaxError(this.state.locale.t('errors.syntax.if'));
      }
