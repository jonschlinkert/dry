...
const { regex: { VariableSignature: v }, utils: { r } } = Dry;
...
class Assign extends Dry.Tag {
  ...
  parse(context) {
    ...
    if (this.Syntax(this.markup, Assign.Syntax)) {
      this.to   = this.last_match[1] || this.last_match[2];
      this.from = this.parse_args(this.last_match[3], context);
    } else {
