...
class Paginate extends BlockTag {
  PaginateSyntax = utils.r`(${regex.QuotedFragment})\\s*(by\\s*(\\d+))?`;

  constructor(node, state) {
    super(node, state);
    this.current_url = '';

    if (this.Syntax(this.value, this.PaginateSyntax)) {
      ...
      this.raise_syntax_error(this.state, 'errors.syntax.paginate');
    }
  }
  ...
  #link(title, page) {
    return { title: title, url: `${this.current_url}?page=${page}`, is_link: true };
  }
}
