...
const { regex: { QuotedFragment: q }, utils } = Dry;
...
class Block extends Dry.BlockTag {
  ...
  lazy_set_block(name, mode, filters) {
    const set = async context => {
      ...
      this.variable_name = await variable.render(context) || name;
      this.state.set_block(this.variable_name, this);
    };
  ...
  async render_block(context) {
    ...
    const render = () => block !== this ? this.render_inner(context) : '';

    switch (mode) {
      case 'append': return await render() + output;
      case 'prepend': return output + await render();
      case 'replace':
      default: {
        return output;
      }
