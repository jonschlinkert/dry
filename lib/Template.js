...
const { RAISE_EXCEPTION_LAMBDA, kInstanceAssigns, kTags } = require('./constants');
...
class Template {
  ...
  parse(input) {
    this.state = this.configure_options(this.options);
    this.root = new Parser(input, this.options, this.state);
    this.root.parse();
    return this;
  }
  ...
  create_context(obj) {
    ...
    if (obj instanceof Dry.Context) {
      if (this.rethrow_errors) {
        obj.exception_renderer = Dry[RAISE_EXCEPTION_LAMBDA];
      }
  ...
  render(...args) {
    ...
    //     drop.context = new Dry.Context(subcontext(drop));
    //     return drop.context;
    //   }

    //   if (isObject(first)) {
    //     const envs = args.shift();
    //     return new Dry.Context(subcontext(envs));
    //   }

    //   if (!first) {
    //     return new Dry.Context(subcontext());
    //   }
    ...
    if (isObject(last)) {
      ...
      if (options.registers) {
        for (const [key, register] of options.registers) {
