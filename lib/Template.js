...
const { RAISE_EXCEPTION_LAMBDA } = require('./constants');
...
class Template {
    ...
    for (const filters of Dry.StrainerFactory.global_filters) {
      for (const [key, fn] of Object.entries(filters)) {
        ...
        const filter = function(...args) {
          ...
          //   // throw new Dry.ArgumentError(length, args.length);
          //   return context.handle_error(new Dry.ArgumentError(length, args.length), {
          //     line_number: loc.line
          //   });
          //   // return new Dry.ArgumentError(length, args.length)
          // }
  ...
  parse(input, options = {}) {
    this.state = this.configure_options(options);
    this.root = new Parser(input, options, this.state);
    this.root.parse();
    return this;
  }
  ...
  render(...args) {
    ...
    const context_registers = context.registers instanceof Map
